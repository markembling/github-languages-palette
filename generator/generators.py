...
from .protocols import PaletteGenerator
...
_GENERATORS: dict[str, type[PaletteGenerator]] = {
    "ccxml": CcxmlGenerator,
    "gpl": GplGenerator,
    "ase": AseGenerator,
    "aco": AcoGenerator,
    "json": JsonGenerator,
...
def generator_for_format(format: str) -> PaletteGenerator:
    """Return the appropriate generator class for the given format"""
    try:
        generator_cls = _GENERATORS[format]
    except KeyError:
        raise ValueError(f"Unknown palette format: {format!r}") from None
